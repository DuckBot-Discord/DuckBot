...
class ImageMan(commands.Cog, name='Image'):
    ...
    async def gay(self, ctx: CustomContext, member: typing.Optional[discord.Member], flag: typing.Optional[flags] = 'gay'):
    ...
    @commands.command()
    async def trash(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Takes out the trash. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.trash()))

    @commands.command()
    async def deepfry(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Deep fries a user. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.deepfry()))

    @commands.command()
    async def ascii(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ *(#@$#%>#%$@%}{:">?$@%@#%$*)@#($. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.ascii()))

    @commands.command()
    async def charcoal(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Applies a charcoal filter to a user's avatar. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.charcoal()))

    @commands.command()
    async def poster(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Posterizes an image. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.poster()))

    @commands.command()
    async def sepia(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Adds a sepia filter to an image. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.sepia()))

    @commands.command()
    async def swirl(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Swirls a member's avatar. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.swirl()))

    @commands.command()
    async def paint(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Adds a painted effect to a member's avatar. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.paint()))

    @commands.command()
    async def night(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ De-saturates a member's avatar. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.night()))

    @commands.command()
    async def rainbow(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Adds a rainbow effect to an image. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.rainbow()))

    @commands.command()
    async def magik(self, ctx: CustomContext, member: typing.Optional[discord.Member]):
        """ Magiks an image. """
        await ctx.send(file=await ctx.dagpi(member, feature=dtype.magik()))

    @commands.command(name='5g1g')
    async def _g_g(self, ctx: CustomContext, guys: discord.Member, girl: discord.Member):
    ...
    async def why_are_g(self, ctx: CustomContext, interviewer: discord.Member, person: discord.Member):
