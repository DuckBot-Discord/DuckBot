...
from lavalink.events import (NodeChangedEvent,
                             PlayerUpdateEvent,
                             QueueEndEvent,
                             TrackEndEvent,
                             TrackExceptionEvent,
...
class QueueMenu(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        ...
        if len(self.data) > 1:
            text = f'{len(self.data)} songs in the queue'
        embed = discord.Embed(title=text, colour=discord.Color.blurple(),
...
class PlayMenu(discord.ui.Select):
    def __init__(self, data, bot, ctx):
        ...
        values.append(discord.SelectOption(label='Cancel',
                                           description='Cancels the action and disconnects the player, '
    ...
    async def callback(self, interaction: discord.Interaction):
        for track in self.info:
            if self.values[0] == track['info']['identifier']:
                ...
                embed = discord.Embed(color=discord.Color.blurple(), title="Added a track to the queue",
    ...
    async def play(self, track: typing.Union[lavalink.AudioTrack, dict] = None, start_time: int = 0, end_time: int = 0,
        ...
        if start_time is not None:
            if not isinstance(start_time, int) or not 0 <= start_time <= track.duration:
                raise ValueError('start_time must be an int with a value equal to, '
...
class Music(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):  # This ensures the client isn't overwritten during cog reloads.
            bot.lavalink = lavalink.Client(bot_user_id, CustomPlayer)
            bot.lavalink.add_node(*lavalink_node_settings)
    ...
    async def cog_command_error(self, ctx, error):
        ...
        if isinstance(error, InvalidTrack):
            embed = discord.Embed(colour=discord.Color.red(),
                                  description="Can't perform action on track that is out of the queu.")
            return await ctx.send(embed=embed)
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState,
        ...
        if not player.channel_id or not player: 
            return self.bot.lavalink.player_manager.remove(member.guild.id)
        channel = self.bot.get_channel(int(player.channel_id))
        ...
        if members > 0:
            if member.id == player.dj:
                if after.channel is None:
                    ...
                    embed = discord.Embed(title=f'New DJ', color=discord.Color.blurple(),
    ...
    async def track_hook(self, event):
        if isinstance(event, QueueEndEvent):
            ...
            embed = discord.Embed(title=f'Inactive player', color=discord.Color.red(),
        ...
        if isinstance(event, TrackStartEvent):
            if event.player.loop != 1:
                ...
                embed = discord.Embed(title=f'Now playing', color=discord.Color.blurple(),
    ...
    async def play_command(self, ctx: commands.Context, *, query: str):
        ...
        if results['loadType'] == 'PLAYLIST_LOADED':
            ...
            embed = discord.Embed(color=discord.Color.blurple(), title="Added a playlist to the queue",
    ...
    async def playing_command(self, ctx: commands.Context):
        ...
        embed = discord.Embed(title=f'Current Track', color=discord.Color.blurple(),
    ...
    async def remove_range_command(self, ctx: commands.Context, start: int, *, end: int = None):
        ...
        if end is None or start == end:
            embed = discord.Embed(color=discord.Color.blurple(),
                                  description=f'Successfully removed **[{player.queue[start - 1].title.upper()}]'
                                              f'({player.queue[start - 1].uri})** from the queue')
            del player.queue[start - 1]
        else:
            embed = discord.Embed(color=discord.Color.blurple(),
                                  description=f'Successfully removed tracks **`{start}`** to **`{end}`** '
                                              f'from the queue ({len(player.queue[start - 1:end - 1])} tracks)')
    ...
    async def move_command(self, ctx: commands.Context, position: int, *, track: str):
        ...
        for x in queue:
            if x['title'].upper() == track.upper():
                ...
                embed = discord.Embed(color=discord.Color.blurple(),
                                      description=f'Successfully moved **[{x["title"]}]({x["uri"]})** '
