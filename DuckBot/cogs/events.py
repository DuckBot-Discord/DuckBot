...
class Handler(commands.Cog, name='Handler'):
    ...
    def cog_unload(self):
        ...
        self.delete_old_records.cancel()
    ...
    async def delete_old_records(self):
        await self.bot.db.execute("DELETE FROM commands WHERE timestamp < now() - interval '30 days'")
    ...
    async def remove_previously_muted(self, member: discord.Member):
        ...
        if role in member.roles:
            await self.bot.db.execute('INSERT INTO muted (user_id, guild_id) VALUES ($1, $2) '
                                      'ON CONFLICT (user_id, guild_id) DO NOTHING', member.id, member.guild.id)
