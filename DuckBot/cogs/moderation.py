...
class Moderation(commands.Cog):
    ...
    async def role_group(self, ctx: CustomContext, member: discord.Member, *, role: discord.Role):
        ...
        await ctx.send(f'✅ **|** Added role **{role}** to **{member}**', allowed_mentions=discord.AllowedMentions.none(), reply=False)
    ...
    async def role_remove(self, ctx: CustomContext, member: discord.Member, *, role: discord.Role):
        ...
        await ctx.send(f'✅ **|** Removed role **{role}** from **{member}**', allowed_mentions=discord.AllowedMentions.none(), reply=False)
    ...
    async def role_all(self, ctx: CustomContext, *, role: discord.Role):
        ...
        await ctx.send(f'⏳ **|** Adding role to all users... **ETA: in {humanize.time.precisedelta((ctx.guild.member_count - len(role.members)) * 1.2)}**', reply=False)
        ...
        await ctx.send(f'✅ **|** Added role **{role}** to all users!', allowed_mentions=discord.AllowedMentions.none(), reply=False)
    ...
    async def role_remove_all(self, ctx: CustomContext, *, role: discord.Role):
        ...
        await ctx.send(f'⏳ **|** Removing role from all users... **ETA: in {humanize.time.precisedelta((ctx.guild.member_count - len(role.members)) * 1.2)}**', reply=False)
        ...
        await ctx.send(f'✅ **|** Removed role **{role}** from all users!', allowed_mentions=discord.AllowedMentions.none(), reply=False)
    ...
    async def role_list(self, ctx: CustomContext):
        """ Lists all roles. """
        roles = sorted([role for role in ctx.guild.roles if not role.is_default()], key=lambda role: role.position, reverse=True)
    ...
    async def role_multi_give(self, ctx: CustomContext, roles: commands.Greedy[discord.Role], *members: discord.Member):
        ...
        for role in roles:
            if role >= ctx.author.top_role and not ctx.guild.owner == ctx.author:
                raise commands.BadArgument(f'❌ **|** Sorry but **you** can\'t add `@{role.name}` because its higher (or equal to) **your top role**!')
            if not role.is_assignable():
                raise commands.BadArgument(f'❌ **|** Sorry but **I** can\'t add the role `@{role.name}`!')

        await ctx.send(f'⏳ **|** Adding roles to all users... **ETA: in {humanize.time.precisedelta((len(members) * len(roles)) * 1.2)}**', reply=False)
