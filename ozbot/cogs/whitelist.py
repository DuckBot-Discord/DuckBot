...
class Whitelist(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        with contextlib.suppress(discord.HTTPException):
            ...
            if await self.bot.db.fetchval('select user_id from usernames where user_id = $1', message.author.id):
                ...
                return await self.bot.get_channel(799741426886901850).send(f"{message.author} tried to add themselves to the whitelist, but they're already in the database.")
        ...
        async with self.bot.session.get(f"https://api.mojang.com/users/profiles/minecraft/{argument}") as r:
            if r.status in (204, 400):
                return await message.channel.send(f"❌ Sorry {message.author.mention} but **`{argument[0:100]}`** is not a valid **__Minecraft Java Edition__** username.", delete_after=20)
                ...
                try:
                    ...
                    if uuid:
                        await message.channel.send(f"Your discord user id is already in the whitelist database. You will receive the roles."
                                                   f"\nIf you cannot log on, warn an admin.", delete_after=10)
                    else:
                        await message.channel.send(f"Your minecraft user id is already in the whitelist database. You will receive the roles."
                                                   f"\nIf you cannot log on, warn an admin.", delete_after=10)
    ...
    async def whitelist_add(self, ctx, member: discord.Member, mc_user: MinecraftName):
        try:
            await self.bot.db.execute("INSERT INTO usernames (user_id, minecraft_id) VALUES ($1, $2)", member.id, mc_user.uuid)
        except asyncpg.UniqueViolationError:
            ...
            if uuid:
                ...
                dc_id = await self.bot.db.fetchval('select user_id from usernames where minecraft_id = $1', mc_user.uuid)
    ...
    async def whitelist_remove(self, ctx: commands.Context, discord_or_minecraft_user: typing.Union[discord.Member, MinecraftName]):
        ...
        if isinstance(member, discord.Member):
            ...
            await ctx.send('something went wrong... idk why. <@349373972103561218> fix it.', allowed_mentions=discord.AllowedMentions(everyone=False, users=True, roles=False, replied_user=False))
