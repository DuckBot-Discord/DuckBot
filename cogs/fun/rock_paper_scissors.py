...
class RockPaperScissors(discord.ui.View):
    ...
    async def update_message(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        embed.description = (
            f"> {self.ctx.default_tick(self.player1.id in self.responses)} {self.player1.display_name}"
        ...
        if len(self.responses) == 2:
            ...
            embed.description = (
                f"> ✅ **{self.player1.display_name}** chose **{self.responses[self.player1.id]}**"
    ...
    async def interaction_check(self, interaction: Interaction) -> bool:
        ...
        if interaction.user.id in self.responses:
            await interaction.response.send_message(
                f'You already selected **{self.responses[interaction.user.id]}**, sorry!', ephemeral=True
            )
            return False
        return True
...
class RockPaperScissorsCommand(FunBase):
    ...
    async def rock_paper_scissors(self, ctx: CustomContext, to_invite: discord.Member = None):
        ...
        if player2:
            embed = discord.Embed(
                description=f"> ❌ {player1.display_name}" f"\n> ❌ {player2.display_name}", colour=discord.Colour.blurple()
            )
            embed.set_author(name='Rock-Paper-Scissors', icon_url='https://i.imgur.com/ZJvaA90.png')
