...
class VoiceLogs(LoggingBase):
    ...
    async def logger_on_voice_state_update(
        ...
        if (
            ...
            and self.bot.guild_loggings[member.guild.id].voice_move
        ):
            embed = discord.Embed(
        ...
        if before.deaf != after.deaf:
            if after.deaf:
                embed = discord.Embed(
                    title='Member Deafened by a Moderator',
        ...
        if before.mute != after.mute:
            if after.mute:
                embed = discord.Embed(
                    title='Member Muted by a Moderator', colour=discord.Colour.dark_gold(), timestamp=discord.utils.utcnow()
                )
                embed.set_author(name=str(member), icon_url=member.display_avatar.url)
                embed.set_footer(text=f"Member ID: {member.id}")
                self.log(embed, guild=member.guild, send_to=self.send_to.voice)
    ...
    async def logger_on_stage_instance_create(self, stage_instance: discord.StageInstance):
        if (
            stage_instance.guild.id not in self.bot.log_channels
            or not self.bot.guild_loggings[stage_instance.guild.id].stage_open
        ):
            return
        embed = discord.Embed(
            title='Stage opened',
    ...
    async def logger_on_stage_instance_delete(self, stage_instance: discord.StageInstance):
        if (
            stage_instance.guild.id not in self.bot.log_channels
            or not self.bot.guild_loggings[stage_instance.guild.id].stage_close
        ):
            return
        embed = discord.Embed(
            title='Stage closed',
