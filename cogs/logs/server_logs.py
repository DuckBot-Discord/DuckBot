...
class ServerLogs(LoggingBase):
    ...
    async def logger_on_guild_channel_delete(self, channel: guild_channels):
        ...
        embed = discord.Embed(
            title=f'{channel.type} channel deleted.'.title(),
        ...
        for target, over in channel.overwrites.items():
            ...
            for perm, value in dict(over).items():
                if value is not None:
                    perms.append(
                        f"{str(perm).replace('guild', 'server').replace('_', ' ').title()} {constants.DEFAULT_TICKS[value]}"
                    )
            if perms:
                ...
                for perm, value in b_o.items():
                    if value != a_o[perm]:
                        updated_perms.append(
                            f"{str(perm).replace('server', 'guild').replace('_', ' ').title()}: {constants.SQUARE_TICKS[value]} ➜ {constants.SQUARE_TICKS[a_o[perm]]}"
                        )
                if updated_perms:
    ...
    async def logger_on_guild_emojis_update(
        self, guild: discord.Guild, before: typing.Sequence[discord.Emoji], after: typing.Sequence[discord.Emoji]
    ):
    ...
    def emoji_update(self, guild: discord.Guild, before: discord.Emoji, after: discord.Emoji):
        ...
        embed = discord.Embed(
            ...
            colour=discord.Colour.blurple(),
            timestamp=discord.utils.utcnow(),
            description=f'{str(after)} | [{after.name}]({after.url})',
        )
    ...
    async def logger_on_guild_stickers_update(
        self, guild: discord.Guild, before: typing.Sequence[discord.Sticker], after: typing.Sequence[discord.Sticker]
    ):
        if guild.id not in self.bot.log_channels:
            return
