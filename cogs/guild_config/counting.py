...
class Counting(ConfigBase):
    ...
    async def ct_set_channel(self, ctx: CustomContext, channel: discord.TextChannel):
        ...
        try:
            ...
            if (ctx.guild.id in self.bot.counting_channels and
                self.bot.counting_channels[ctx.guild.id]['channel'] != channel.id) or \
                    (ctx.guild.id not in self.bot.counting_channels):
