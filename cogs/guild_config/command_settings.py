...
class PlonkedPageSource(menus.AsyncIteratorPageSource):
    ...
    async def format_page(self, menu: ViewPaginator, entries: list[str]):
...
class ResolvedCommandPermissions:
    ...
    def is_blocked(self, ctx: CustomContext) -> Optional[bool]:
        # fast path
        if len(self._lookup) == 0:
            return False
...
class CommandConfigs(ConfigBase):
    ...
    async def ignore(self, ctx: CustomContext, *entities: Union[discord.TextChannel, discord.Member, discord.VoiceChannel]):
    ...
    async def ignore_list(self, ctx: CustomContext):
        ...
        pages = ViewPaginator(source, ctx=ctx)
        await pages.start()

    @ignore.command(name='all')
    @commands.has_guild_permissions(ban_members=True, manage_messages=True)
    async def _all(self, ctx: CustomContext):
    ...
    @commands.has_guild_permissions(ban_members=True, manage_messages=True)
    async def ignore_clear(self, ctx: CustomContext):
    ...
    @commands.has_guild_permissions(ban_members=True, manage_messages=True)
    async def unignore(self, ctx: CustomContext, *entities: Union[discord.TextChannel, discord.Member, discord.VoiceChannel]):
    ...
    @commands.has_guild_permissions(ban_members=True, manage_messages=True)
    async def unignore_all(self, ctx: CustomContext):
    ...
    @config.group()
    @commands.has_guild_permissions(ban_members=True, manage_messages=True)
    async def channel(self, ctx: CustomContext):
    ...
    async def channel_disable(self, ctx: CustomContext, *, command: CommandName):
        """Disables a command for this channel."""

        try:
            await self.command_toggle(ctx.bot.db, ctx.guild.id, ctx.channel.id, command, whitelist=False)
    ...
    async def channel_enable(self, ctx: CustomContext, *, command: CommandName):
        """Enables a command for this channel."""

        try:
            await self.command_toggle(ctx.bot.db, ctx.guild.id, ctx.channel.id, command, whitelist=True)
        except RuntimeError as e:
            await ctx.send(str(e))
        else:
    ...
    async def server_disable(self, ctx: CustomContext, *, command: CommandName):
        """Disables a command for this server."""

        try:
            await self.command_toggle(ctx.bot.db, ctx.guild.id, None, command, whitelist=False)
        except RuntimeError as e:
            await ctx.send(str(e))
        else:
    ...
    async def server_enable(self, ctx: CustomContext, *, command: CommandName):
        """Enables a command for this server."""

        try:
            await self.command_toggle(ctx.bot.db, ctx.guild.id, None, command, whitelist=True)
        except RuntimeError as e:
            await ctx.send(str(e))
        else:
    ...
    @commands.has_guild_permissions(ban_members=True, manage_messages=True)
    async def config_enable(self, ctx: CustomContext, channel: Optional[discord.TextChannel], *, command: CommandName):
        """Enables a command the server or a channel."""

        channel_id = channel.id if channel else None
        human_friendly = channel.mention if channel else 'the server'
        try:
            await self.command_toggle(ctx.bot.db, ctx.guild.id, channel_id, command, whitelist=True)
        except RuntimeError as e:
            await ctx.send(str(e))
        else:
    ...
    @commands.has_guild_permissions(ban_members=True, manage_messages=True)
    async def config_disable(self, ctx: CustomContext, channel: Optional[discord.TextChannel], *, command: CommandName):
        """Disables a command for the server or a channel."""

        channel_id = channel.id if channel else None
        human_friendly = channel.mention if channel else 'the server'
        try:
            await self.command_toggle(ctx.bot.db, ctx.guild.id, channel_id, command, whitelist=False)
        except RuntimeError as e:
            await ctx.send(str(e))
        else:
    ...
    async def config_disabled(
        self, ctx: CustomContext, *, channel: Optional[Union[discord.TextChannel, discord.VoiceChannel]] = None
    ):
