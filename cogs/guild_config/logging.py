...
styles = {True: discord.ButtonStyle.green,
...
class EventToggle(discord.ui.Button['AllEvents']):
    def __init__(self, event: str, enabled: bool):
        super().__init__(label=event.title().replace('_', ' ').replace('guild', 'server'), style=styles[enabled])
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        q = await self.view.ctx.bot.db.fetch(f'UPDATE logging_events SET {self.event} = $1 WHERE guild_id = $2 RETURNING {self.event}',
                                               self.enabled, self.view.ctx.guild.id)
        if not q:
        ...
        setattr(self.view.ctx.bot.guild_loggings[self.view.ctx.guild.id], self.event, self.enabled)
...
class AllEvents(discord.ui.View):
    ...
    def prepare(self):
        events = {'message': '📨', 'join_leave': '👋', 'member': '👤', 'voice': '🎙', 'server': '⚙'}
