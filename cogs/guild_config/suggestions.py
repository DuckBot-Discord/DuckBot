...
class Suggestions(ConfigBase):
    ...
    async def enable_suggestions(self, ctx: CustomContext, channel: discord.TextChannel, image_only: bool):
        """
        ...
        **Note:** If image-only is set to `yes`, the bot will delete all messages without attachments, and warn the user.
        ...
        await self.bot.db.execute(
            'INSERT INTO suggestions (channel_id, image_only) VALUES ($1, $2) ON CONFLICT '
            '(channel_id) DO UPDATE SET image_only = $2',
            channel.id,
            image_only,
        )
        await ctx.send(
            f'💞 | **Enabled** suggestions mode for {channel.mention}'
    ...
    async def disable_suggestions(self, ctx: CustomContext, channel: discord.TextChannel):
        """
        ...
        await ctx.send(f'💞 | **Disabled** suggestions mode for {channel.mention}' f'\n📸 | With image-only mode **N/A**.')
