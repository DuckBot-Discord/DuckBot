...
class EarnMoney(EconomyBase):
    ...
    async def work(self, ctx: CustomContext):
        ...
        async with ctx.wallet as wallet:
            ...
            await ctx.send(
                random.choice(self.work_messages).format(coin=self.coin_emoji, amount=amount, currency=self.coin_name)
            )
    ...
    async def daily(self, ctx: CustomContext):
        ...
        async with ctx.wallet as wallet:
            ...
            await wallet.add_money(amount)
            await ctx.send(
                f"{self.coin_emoji} **{ctx.me.name}** hands you **{amount} {self.coin_name}** as your daily reward."
            )
    ...
    async def weekly(self, ctx: CustomContext):
        ...
        async with ctx.wallet as wallet:
            ...
            await wallet.add_money(amount)
            await ctx.send(
                f"{self.coin_emoji} **{ctx.me.name}** hands you **{amount} {self.coin_name}** as your weekly reward."
            )
    ...
    async def monthly(self, ctx: CustomContext):
        ...
        async with ctx.wallet as wallet:
            ...
            await wallet.add_money(amount)
            await ctx.send(
                f"{self.coin_emoji} **{ctx.me.name}** hands you **{amount} {self.coin_name}** as your monthly reward."
            )
    ...
    def bet(self, bet: typing.Union[int, str], wallet: Wallet):
        ...
        if bet > wallet.balance:
            raise commands.BadArgument(
                f'❗ You cannot bet more than your current balance of **{wallet.balance} {self.coin_name}**.'
            )
        if bet < 0:
    ...
    @commands.command()
    async def race(
        self, ctx: CustomContext, duck: int = None, bet: typing.Union[int, str] = None, fast_forward: bool = False
    ):
        ...
        async with ctx.wallet as wallet:
            if not all(x is not None for x in (duck, bet)):
                raise commands.BadArgument(
                    '❗ You must pass all the arguments. '
            ...
            embed = discord.Embed(title="🐣 Duck race!", description='\n'.join(map(str, ducks)))

            embed.add_field(name='Your bet:', value=f'You bet **{bet} {self.coin_name}** for duck **{duck}**.')
            ...
            while not any(d.has_won for d in ducks):
                ...
                embed.clear_fields()
                embed.add_field(
                    name='Your bet', value=f'You bet **{bet} {self.coin_name}** for duck **{duck}**.' f'\n{content}'
                )
