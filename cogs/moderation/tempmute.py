...
from utils import DuckCog, DuckContext, UserFriendlyTime, human_timedelta, command, group

from utils import HierarchyException, MemberNotMuted, MemberAlreadyMuted, Timer
...
class TempMute(DuckCog):
        ...
        async with self.bot.safe_connection() as connection:
            ...
            if days_out <= datetime.timedelta(days=28):
                ...
                await connection.execute(
                    'UPDATE guilds SET mutes = array_append(mutes, $1) WHERE guild_id = $2', member.id, guild.id
                )
    ...
    async def cache_validation(self, before: discord.Member, after: discord.Member) -> None:
        ...
        async with self.bot.safe_connection() as conn:
            ...
            if after.id in mutes:
                await conn.execute(
                    'UPDATE guilds SET mutes = array_remove(mutes, $1) WHERE guild_id = $2', after.id, after.guild.id
                )
    ...
    async def mute_dispatcher(self, member_id: int, guild_id: int, *, roles: List[int]) -> None:
        ...
        ):  # The bot doesn't have the permissions to manage roles anymore, do nothing.
            return log.debug(
