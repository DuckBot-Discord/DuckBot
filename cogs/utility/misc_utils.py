...
class MiscUtils(UtilityBase):
    ...
    async def minecraft_uuid(self, ctx: CustomContext, *, username: str) -> typing.Optional[discord.Message]:
    ...
    async def _in_command(self, ctx, *, relative_time: time_inputs.ShortTime):
        ...
        await ctx.send(
            f"{discord.utils.format_dt(relative_time.dt, style='F')} "
    ...
    async def permissions(
        ...
        for perm, value in target.guild_permissions:
            perms.append([perm.replace('guild', 'server').replace('_', ' ').title(), str(value), str(perm in channel_perms)])
