...
class ReactionHandling(EventsBase):
    ...
    async def wastebasket(self, payload: discord.RawReactionActionEvent):
        if (
            payload.channel_id == self.error_channel
            and await self.bot.is_owner(payload.member)
        ...
        if (
            payload.channel_id == suggestions_channel
            and await self.bot.is_owner(payload.member)
            ...
            sub = {'Suggestion ': 'suggestion', 'suggestion ': 'suggestion', 'Denied ': '', 'Approved ': ''}
            ...
            scheme = {'🔼': (0x6AED64, f'Approved suggestion {title}'), '🔽': (0xF25050, f'Denied suggestion {title}')}[
                str(payload.emoji)
            ]
            ...
            if str(payload.emoji) == '🔼' and user_id:
                try:
                    ...
                    user_embed = discord.Embed(
                        ...
                        description=f"**Your suggestion has been approved! " f"You suggested:**\n{suggestion}",
                    )
                    user_embed.set_footer(
                        text='Reply to this DM if you want to stay in contact ' 'with us while we work on your suggestion!'
                    )
