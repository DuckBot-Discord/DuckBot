...
class fun(commands.Cog):
    ...
    @commands.command(help="shows a funny \"inspirational\" image (generated by AI)")
    @commands.bot_has_permissions(send_messages=True, embed_links=True)
    @commands.check_any(commands.has_role(864737541726142474), commands.has_permissions(manage_messages=True))
    async def inspireme(self, ctx):
        async with aiohttp.ClientSession() as cs:
            async with cs.get('http://inspirobot.me/api?generate=true') as r:
                res = await r.text()
                embed = discord.Embed(title='An inspirational image... (Sarcasm! do not take this seriously)', color=random.randint(0, 0xFFFFFF))
                embed.set_image(url=res)
                embed.set_footer(text='by inspirobot.me | These images are for entretainment purposes only, and generated by an artificial intelligence. Do not take seriously!', icon_url='https://inspirobot.me/website/images/inspirobot-dark-green.png')
                await ctx.send(embed=embed)
