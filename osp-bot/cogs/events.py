...
You have 5 minutes to do so.""")
                ...
                embed.set_author(name=f"{payload.member} is leaving the ticket",
...
{msg.content}""")
                ...
                await message.channel.set_permissions(payload.member, overwrite=perms,
                                                      reason=f"{payload.member.name} left ticket")
                # LOG
...
{payload.member.mention} left ticket: {message.channel.mention}""")
                ...
                logemb.set_author(name=str(payload.member), icon_url=payload.member.display_avatar.url)
            ...
            if payload.channel_id in chids:
                ...
                # LOG
                TicketLog = await self.get_webhook(self.ticket_log)
                logemb = discord.Embed(color=0x4286F4, title=f"Ticket #{message.channel.name} archived",
                                       description=f"""
                ...
                overwrites = {
                    ...
                    self.ticket_staff: discord.PermissionOverwrite(read_messages=True, manage_messages=False,
                                                                   send_messages=False)
                }
    ...
    async def on_member_join(self, member):
        ...
        await self.bot.get_channel(self.yaml_data['JLLog']).send(
            f"""<:outgoingarrow:848312880679354368> **{member.name}#{member.discriminator}** joined **{member.guild.name}**!""")
        await member.add_roles(self.mguild.get_role(860612731102822400), self.mguild.get_role(860609206453665792),
                               self.mguild.get_role(860613002906697800), self.mguild.get_role(860612542619189249),
                               self.mguild.get_role(867094081241612308), self.mguild.get_role(867096893863624734),
                               self.mguild.get_role(860613615811166219),
                               reason=f"autoroles automatically applied to {member}")

    @commands.Cog.listener()
    async def on_member_remove(self, member):
        if member.guild.id != self.yaml_data['guildID']: return
        await self.bot.get_channel(self.yaml_data['JLLog']).send(
            f"""<:incomingarrow:848312881070080001> **{member.name}#{member.discriminator}** left **{member.guild.name}**!""")
...
"""
    ...
    async def on_trigger_word(self, message):
        if not message.guild or message.guild.id != 831897006812561409:
            return
        ...
        suicide_embed.add_field(name="National Suicide Prevention Lifeline", value="1-800-273-TALK (8255)",
                                inline=False)
        suicide_embed.add_field(name="American Association of Suicidology",
                                value="The [American Association of Suicidology](http://www.suicidology.org/) (AAS) promotes research, public awareness programs, public education and training for professionals and volunteers. AAS also serves as a national clearinghouse for information on suicide.",
                                inline=False)
        suicide_embed.add_field(name="Depression Screening",
                                value="The [Depression Screening](http://www.depression-screening.org/) website is sponsored by Mental Health America as part of the Campaign for America’s Mental Health. The mission of this website is to educate people about clinical depression, offer a confidential way for people to get screened for symptoms of depression and guide people toward appropriate professional help if necessary.",
                                inline=False)
        suicide_embed.add_field(name="MoodGYM",
                                value="[MoodGYM](http://www.moodgym.anu.edu.au/) has been evaluated in a scientific trial and found to be effective in relieving depression symptoms if people work through it systematically. This website uses cognitive behavioral therapy (CBT) methods to teach people to use ways of thinking that can help prevent depression.",
                                inline=False)
        suicide_embed.add_field(name="Progressive Relaxation",
                                value="[Download](http://www.hws.edu/studentlife/counseling_relax.aspx) two progressive relaxation tapes from the Hobart and William Smith Colleges website.",
                                inline=False)
        suicide_embed.add_field(name="Suicide Prevention Resource Center",
                                value="[The Suicide Prevention Resource Center](http://www.sprc.org/) has fact sheets on suicide by state and by population characteristics, as well as on many other subjects.",
                                inline=False)
        ...
        self_injury_embed.add_field(name="Focus Adolescent Services",
                                    value="The [Focus Adolescent Services](http://www.focusas.com/) website is designed for parents and covers a wide range of mental health problems, including a section on self-injury.",
                                    inline=False)
        self_injury_embed.add_field(name="S.A.F.E. Alternatives (Self-Abuse Finally Ends)",
                                    value="[S.A.F.E. Alternatives](http://www.selfinjury.com/) is a residential treatment program for people who engage in self-injury. The website includes information about self-injury and about starting treatment. S.A.F.E information line: (Phone Word Acronym is a TW) ||1-800-DONT-CUT|| (1-800-366-8288)",
                                    inline=False)

        if "help" in message.content.lower() and self.bot.user in message.mentions:
            await message.channel.send(
                f"hello, {message.author.mention}! If you need assistance, please DM me and an admin will assist you!")
        ...
        elif self.bot.user in message.mentions and message and len(
                message.content) <= 22 and not message.content.startswith("."):
