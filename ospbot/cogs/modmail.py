...
class modmail(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if message.content:
            embed.add_field(name=f'{constants.LEFT_SERVER} **{message.author}**', value=f'{message.content}')
        else:
            embed.add_field(name=f'{constants.LEFT_SERVER} **{message.author}**', value=f'_ _')
    ...
    async def dm(self, ctx, member: typing.Optional[discord.Member], *, message = None):
        ...
        try:
            if ctx.message.attachments:
                ...
                if message:
                    embed.add_field(name=f'{constants.JOINED_SERVER} **{member.name}#{member.discriminator}**', value=message)
                    await member.send(message, file=myfile)
                else:
                    embed.add_field(name=f'{constants.JOINED_SERVER} **{member.name}#{member.discriminator}**', value='_ _')
